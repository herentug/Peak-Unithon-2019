...
namespace Utils.SortingLayer.Editor
{
    ...
    public class SortingLayerExposedEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            var sortingLayerNames = UnityEngine.SortingLayer.layers.Select(l => l.name).ToArray();

            // Look up the layer name using the current layer ID
            string oldName = UnityEngine.SortingLayer.IDToName(renderer.sortingLayerID);
            ...
            if (newLayerIndex != oldLayerIndex) {
                ...
                renderer.sortingLayerID = UnityEngine.SortingLayer.NameToID(sortingLayerNames[newLayerIndex]);
